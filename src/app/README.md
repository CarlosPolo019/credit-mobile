...
- `AppRouter.tsx`: renderiza `OfflineBanner` (arriba de todas las pantallas) y selecciona stack autenticado (`Home`, `CreditCreate`, `CreditList`, `CreditDetail`, `CreditEdit`, `ClientList`, `EmailJobList`, `Dashboard`) o publico (solo `Login` — sin auto-registro, ver `pages/login/README.md`); el stack publico se envuelve en `BackendWakeGate`, el autenticado no (ver mas abajo); oculta headers nativos.
